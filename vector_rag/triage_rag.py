...
Run this ONCE to populate VectorAI DB before starting the dashboard.
"""
...
# ── CONFIG ─────────────────────────────────────────────────────────────────────
HRP_FILE        = "../assets/humanitarian-response-plans.csv"
RELIEFWEB_FILE  = "../assets/data_reliefweb_crisis_figures_data.csv"
FTS_FILE        = "../assets/fts_requirements_funding_global.csv"
VECTORIZER_FILE = "triage_vectorizer.pkl"  # saved in vector_rag/ folder
...
doc_df = doc_df.drop_duplicates(subset=['text']).reset_index(drop=True)
...
vectorizer = TfidfVectorizer(
    max_features=384,   # vector dimension — must match DIMENSION above
...
# Save vectorizer — needed by actian_search.py at query time
...
with CortexClient("localhost:50051") as client:
    ...
    # Delete and recreate collection to avoid stale/duplicate data
    try:
...
def find_comparable_crises(country: str,
    ...
    funding_status = (
        ...
        "Severely underfunded"   if funding_coverage_pct < 60 else
        "Partially funded"       if funding_coverage_pct < 80 else
        "Successfully funded"
    )
